...
public:
    ...
    {   
        ...
        *
        */
        double x = state3D->values[0];  // 获取X坐标
        double y = state3D->values[1];  // 获取Y坐标  
        double z = state3D->values[2];  // 获取Z坐标
...
void pathFinding(const Vector3d start_pt, const Vector3d target_pt)
{
    ...
    ob::StateSpacePtr space(new ob::RealVectorStateSpace(3));  //构造三维状态空间

    // Set the bounds of space to be in [0,1].
    ob::RealVectorBounds bounds(3);    // 边界条件设置
    ...
    space->as<ob::RealVectorStateSpace>()->setBounds(bounds);  //先将space类型转换为RealVectorStateSpace类型，再设置边界
    // Construct a space information instance for this state space
    ob::SpaceInformationPtr si(new ob::SpaceInformation(space));  // 创建空间信息对象
    // Set the object used to check which states in the space are valid
    si->setStateValidityChecker(ob::StateValidityCheckerPtr(new ValidityChecker(si))); // 设置碰撞检测器
    si->setup();
